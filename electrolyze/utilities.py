...
def read_input_file():
    ...
    parser.add_argument(
        "--file",
        "-f",
        help="settings file. Default input_electrolyze",
        default="input_electrolyze",
    )
...
def fit_model(estimators, df, features, target, test_size, n_trials):
    ...
    best_ind = np.where(errors == np.min(errors))[0][
        0
    ]  # select the lowest-error model index
    ...
    ax.set_ylabel(
        "Predicted {}".format(
            "Number of Cycles" if target == "Measurement-3" else target
        )
    )
    ax.set_xlabel(
        "Actual {}".format("Number of Cycles" if target == "Measurement-3" else target)
    )
...
def bayesian_optimize(
    objective_func, bounds, constraints, max_time, max_iter, tolerance
):
    """
    ...
    bo_model = GPyOpt.models.GPModel(
    ...
    acquisition = GPyOpt.acquisitions.AcquisitionEI(
        bo_model, feasible_region, optimizer=aquisition_optimizer
    )
    # Select collection method
    evaluator = GPyOpt.core.evaluators.Sequential(acquisition)
    # Complete optimizer object
    optimizer = GPyOpt.methods.ModularBayesianOptimization(
        bo_model, feasible_region, objective, acquisition, evaluator, initial_design
    )

    # Run optimization
    optimizer.run_optimization(
        max_iter=max_iter, max_time=max_time, eps=tolerance, verbosity=False
    )
